...
class FCOSOutputs(nn.Module):
    def __init__(self, cfg):
        ...
        self.out_len = [self.svd_k * self.num_bases * s for s in self.m_size]
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, mask_pred, gt_instances, top_feats=None):
        """
        ...
        instances.U_pred = cat([
                # Reshape:(N, mask_len, Hi, Wi) -> (N*Hi*Wi, mask_len)
                x.permute(0, 2, 3, 1).reshape(-1, self.out_len[0]) for x in mask_pred['U']
            ])
        instances.V_pred = cat([
                # Reshape:(N, mask_len, Hi, Wi) -> (N*Hi*Wi, mask_len)
                x.permute(0, 2, 3, 1).reshape(-1, self.out_len[1]) for x in mask_pred['V']
            ])
        instances.S_pred = cat([
                # Reshape:(N, mask_len, Hi, Wi) -> (N*Hi*Wi, mask_len)
                x.permute(0, 2, 3, 1).reshape(-1, self.svd_k * self.num_bases) for x in mask_pred['S']
            ])
    ...
    def predict_proposals(
            ...
            locations, mask_pred, image_sizes, top_feats=None
    ):
        if self.training:
        ...
        bundle = {
            ...
            "s": self.strides, "u": mask_pred['U'],
            "v": mask_pred['V'], "i": mask_pred['S'],
        }
    ...
    def forward_for_single_feature_map(
            ...
            ctrness_pred, image_sizes, mask_pred, top_feat=None
    ):
        ...
        for k,v in mask_pred.items():
            v = v.view(N, -1, H, W).permute(0, 2, 3, 1)
            v = v.reshape(N, H*W, -1)
            mask_pred[k] = v
        ...
        for i in range(N):
            ...
            per_u_pred = mask_pred['u'][i]
            per_u_pred = per_u_pred[per_box_loc]
            per_v_pred = mask_pred['v'][i]
            per_v_pred = per_v_pred[per_box_loc]
            per_s_pred = mask_pred['s'][i]
            per_s_pred = per_s_pred[per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                per_u_pred = per_u_pred[top_k_indices]
                per_v_pred = per_v_pred[top_k_indices]
                per_s_pred = per_s_pred[top_k_indices]
            ...
            boxlist.U_pred = per_u_pred
            boxlist.V_pred = per_v_pred
            boxlist.S_pred = per_s_pred
            if top_feat is not None:
